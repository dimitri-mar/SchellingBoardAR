...
def generate_cell_imgs_vect(img_corrected, grid_x, grid_y):
    ...
    dataset  = img_corrected.reshape(img_corrected.shape[1]//target_size[1], target_size[1],
                      img_corrected.shape[0]//target_size[0], target_size[0], img_corrected.shape[2])
    dataset = dataset.swapaxes(1, 2)

    dataset = dataset.reshape(-1, target_size[1], target_size[0], img_corrected.shape[2])
...
def detect_labels_fast(corrected_image,  grid_x, grid_y, model, return_label_img=False):
    ...
    label_matrix = most_probable_classes.reshape((grid_x, grid_y)).T
    ...
    if return_label_img:
        ...
        for x_ix in range(grid_x):
            for y_ix in range(grid_y):
                ...
                cv2.putText(img2,
                            int2label[label_matrix[x_ix, y_ix]],
                            #str(label_matrix.T[x_ix, y_ix]),
                            (int(x), int(y)),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.1, (36, 255, 12), 4)
        # plt.figure(figsize=(26, 18))
        # plt.imshow(img2)
